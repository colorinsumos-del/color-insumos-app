...
if not st.session_state.auth:
    ...
    if menu == "🚪 Salir":
        ...
        file = st.file_uploader("Sube el PDF con imágenes y precios", type="pdf")
        if file and st.button("🚀 Procesar e Importar"):
            with st.spinner("Extrayendo datos e imágenes..."):
                try:
                    total = procesar_pdf_avanzado(file)
                    st.success(f"Éxito: Se procesaron {total} productos.")
